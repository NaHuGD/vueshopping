<template>
    <div>
            ...
            <div class="col-md-4 mb-4" v-for="item in products" :key="item.id">
                <div class="card border-0 shadow-sm">
                    ...
                    <button type="button" class="btn btn-outline-secondary btn-sm"
                        @click="getProduct(item.id)"><!--帶入vfor=>item的id-->
      ...
      <div class="modal-dialog" role="document">
        ...
        <div class="my-5-row justify-content-center"><!--購物車呈現-->
            <div class="col-md-6" style="margin:0 auto;">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="item in cart.carts" :key="item.id">
                            ...
                            <td>
                                {{item.product.title}}
                                <div class="text-success" v-if="item.coupon" style="font-size:.4rem;">
                                    已套用優惠券
                                </div>
                            </td>
                ...
                <div class="text-center">
                    <span v-if="couponMessage == '找不到優惠券!' ||
                    couponMessage == '優惠券無法使用'" class="text-danger">{{couponMessage}}</span>
...
export default {
    ...
    methods:{
        ...
        addtoCart(id,qty = 1){//加入購物車，傳入值預設為1
            const vm = this;
        ...
        createOrder(){ //創建訂單
            ...
            this.$validator.validate().then((result) => {
                if (result) { //email格式正確時發送訂單
                    this.$http.post(url,{data: order }).then((response) => {
                        console.log('訂單',response);
                        if(response.data.success){//訂單確認建立 導頁至結帳畫面

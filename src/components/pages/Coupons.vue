...
export default {
    ...
    methods:{
        openCouponModal(isNew,item){ //新建編輯表單
            const vm = this;
        ...
        removeCoupon(item){ //刪除優惠卷
            const vm = this;
            const url = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/admin/coupon/${item.id}`;
            vm.isLoading = true;
            this.$http.delete(url).then((response) => {
                console.log('刪除優惠卷',response);
                vm.coupons = response.data.coupons;
                vm.getCoupon();
                vm.isLoading = false;
            });
        },
        getCoupon(page = 1){ //取得資料
          const vm = this;
          const url = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/admin/coupons?page=${page}`;
          vm.isLoading = true;
          this.$http.get(url,vm.tempProduct).then((response) => {
              console.log('取得優惠卷',response);
              vm.coupons = response.data.coupons;

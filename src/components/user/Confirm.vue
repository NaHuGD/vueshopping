  ...
  <div class="pt150">
    ...
    <div class="pt150 m-auto col-10">
      <div id="Confirm">
        ...
        <div class="pt-2" :class="{'text-danger':order.is_paid=false,'text-success':order.is_paid=true}">
          <p class="d-inline">付款狀態 :</p>
          <p class="d-inline px-2" v-if="order.is_paid=true">完成付款</p>
          <p class="d-inline px-2" v-else>尚未付款</p>
        </div>
        <button class="paying" @click.prevent="paying" :class="{'active':order.is_paid=false,'active':order.is_paid=true}">
          <span class="box">
...
export default {
  ...
  methods: {
    ...
    paying() {
      ...
      if(vm.payingCheck == false){
        document.querySelector(".paying").classList.add("active");
        setTimeout(function() {
          document.querySelector(".paying .box").innerHTML=`<i class="fas fa-check"></i>`;
          document.querySelector(".paying p").innerHTML = `結帳完成`;
        },600);
        const vm = this;
        ...
        this.$http.post(url).then((response) => {
            console.log(response);
            if(response.data.success){  //付款完成時
